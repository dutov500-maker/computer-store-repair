    ...
    <div className="min-h-screen bg-[#0A0A0A] text-white page-transition">
        ...
        <div className="container mx-auto px-6">
          <div className="max-w-3xl">
            ...
            <h1 className="font-heading text-5xl md:text-7xl font-black uppercase leading-none">
              Галерея
              <br />
              <span className="text-[#FF6B00]">работ</span>
            </h1>
            <p className="text-white/60 text-lg mt-6 max-w-xl">
              Живые фото наших сборок и ремонтных кейсов. Реальная работа — без стоковых картинок.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="py-16 container mx-auto px-6">
        <div className="mb-10 flex justify-center">
          <Tabs value={filter} onValueChange={(v) => setFilter(v as 'all' | 'builds' | 'repairs')} className="w-auto">
            <TabsList className="bg-[#0D0D0D] border border-white/10 p-1 rounded-none">
              <TabsTrigger
                value="all"
                className="rounded-none font-mono text-xs tracking-widest uppercase data-[state=active]:bg-[#FF6B00] data-[state=active]:text-black"
              >
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {filteredImages.map((image, index) => (
            <div
              key={index}
              className="group relative overflow-hidden border border-white/10 bg-[#0D0D0D] hover:border-[#FF6B00]/60 transition-all cursor-pointer"
            ...
            >
              ...
              <div className="absolute bottom-0 left-0 right-0 translate-y-full p-6 transition-transform duration-300 group-hover:translate-y-0">
                <h3 className="font-heading font-black uppercase text-white mb-2">
                  {image.title}
                </h3>
              ...
              <div className="absolute right-3 top-3 opacity-0 group-hover:opacity-100 transition-opacity">
                <Button
                  size="icon"
                  className="h-9 w-9 rounded-none bg-[#FF6B00] hover:bg-[#FF8A2E] text-black"
                >
        ...
        <Dialog open={!!selectedImage} onOpenChange={() => setSelectedImage(null)}>
          <DialogContent className="max-w-5xl border-2 border-[#FF6B00]/40 bg-[#0A0A0A] p-0 rounded-none">
            {selectedImage && (
              <div className="flex flex-col">
                ...
                <div className="p-6 border-t border-white/10">
                  ...
                  <h2 className="font-heading text-2xl font-black uppercase text-white mb-2">
                    {selectedImage.title}
                  </h2>
